			...
			if ( in_array( $ptype, $team_post_types, true ) ) {
				...
				if ( function_exists( 'jjc_mh_get_team_member_ids' ) ) {
					...
					if ( is_array( $members ) && ! empty( $members ) ) {
						foreach ( $members as $m ) $expanded[] = intval( $m );
						// DO NOT append the team id itself — we want individual IDs only for expanded lists
						continue;
					}
				}

				// 2) other known expansion helpers
				if ( function_exists( 'jjc_mh_expand_team_to_members' ) ) {
					$members = jjc_mh_expand_team_to_members( $pid );
					if ( is_array( $members ) && ! empty( $members ) ) {
						foreach ( $members as $m ) $expanded[] = intval( $m );
						continue;
					}
				...
				if ( function_exists( 'wfmc_expand_team_safe' ) ) {
					$members = wfmc_expand_team_safe( $pid );
					if ( is_array( $members ) && ! empty( $members ) ) {
						foreach ( $members as $m ) $expanded[] = intval( $m );
						continue;
					}
				...
				// 3) fallback: read common team postmeta keys
				$possible_team_keys = array( 'team_members', '_team_members', 'members', 'member_ids', 'members_list', 'roster', 'team_roster', 'wf_team_members' );
				foreach ( $possible_team_keys as $tk ) {
					$tm = get_post_meta( $pid, $tk, true );
					if ( empty( $tm ) ) continue;
					// array already
					if ( is_array( $tm ) && ! empty( $tm ) ) {
						foreach ( $tm as $m ) if ( is_numeric( $m ) ) $expanded[] = intval( $m );
						continue 2;
					}
					// serialized / maybe_unserialize
					$dec = @maybe_unserialize( $tm );
					if ( is_array( $dec ) && ! empty( $dec ) ) {
						foreach ( $dec as $m ) if ( is_numeric( $m ) ) $expanded[] = intval( $m );
						continue 2;
					}
					// json
					$json = @json_decode( $tm, true );
					if ( is_array( $json ) && ! empty( $json ) ) {
						foreach ( $json as $m ) if ( is_numeric( $m ) ) $expanded[] = intval( $m );
						continue 2;
					}
					// single numeric string
					if ( is_string( $tm ) && ctype_digit( $tm ) ) {
						$expanded[] = intval( $tm );
						continue 2;
					}
				}

				// 4) last resort: look at wf_match_snapshot applied_participants for match-level per-id info
				...
				if ( is_string( $snap ) ) {
					...
					if ( is_array( $dec ) && isset( $dec['applied_participants'] ) && is_array( $dec['applied_participants'] ) ) {
						...
						// continue (we don't know which ids came from which team, so keep de-duplication later)
					}
				}
			} else {
				$expanded[] = $pid;
...
function wf_sync_match_participants_meta( $match_id ) {
    ...
    // Prefer expanded individual participant IDs (expands teams -> members) when available.
    if ( function_exists( 'wf_expand_match_participants_to_individuals' ) ) {
        $pids = wf_expand_match_participants_to_individuals( $match_id );
    ...
    // also write the explicit expanded key for listing queries
    update_post_meta( $match_id, 'match_participants_expanded', $pids );
...
function wf_recompute_superstar_counters( $superstar_id ) {
    ...
    // Include expanded and snapshot keys so team-only matches are found
    $keys = array( 'participants_details', 'wf_winners', 'match_participants', 'match_players', 'participants', 'participants_list', 'match_participants_expanded', 'wf_match_snapshot' );
    foreach ( $keys as $k ) {
    ...
    foreach ( $match_ids as $mid ) {
        ...
        // Defensive: If this match doesn't include superstar in expanded participants,
        // check snapshot or expanded meta and expanded winners before skipping.
        $contains_superstar = in_array( $superstar_id, $participant_ids, true );

        if ( ! $contains_superstar ) {
            // 1) Check explicit expanded meta
            $exp_meta = get_post_meta( $mid, 'match_participants_expanded', true );
            if ( is_array( $exp_meta ) && in_array( $superstar_id, array_map( 'intval', $exp_meta ), true ) ) {
                $contains_superstar = true;
            } elseif ( is_string( $exp_meta ) && strpos( $exp_meta, '"' . $superstar_id . '"' ) !== false ) {
                $contains_superstar = true;
            }

            // 2) Check snapshot participant_ids and winners_expanded
            if ( ! $contains_superstar ) {
                $snap = get_post_meta( $mid, 'wf_match_snapshot', true );
                if ( is_string( $snap ) ) {
                    $dec = @json_decode( $snap, true );
                    if ( is_array( $dec ) ) {
                        if ( isset( $dec['participant_ids'] ) && is_array( $dec['participant_ids'] ) ) {
                            foreach ( $dec['participant_ids'] as $pid ) {
                                if ( intval( $pid ) === $superstar_id ) { $contains_superstar = true; break; }
                            }
                        }
                        if ( ! $contains_superstar && isset( $dec['winners_expanded'] ) && is_array( $dec['winners_expanded'] ) ) {
                            foreach ( $dec['winners_expanded'] as $wid ) {
                                if ( intval( $wid ) === $superstar_id ) { $contains_superstar = true; break; }
                            }
                        }
                    }
                }
            ...
            // 3) Check legacy wf_winners expansion (team winners -> members)
            if ( ! $contains_superstar ) {
                    ...
                    foreach ( $winners as $w ) {
                        ...
                        if ( in_array( $ptype, array( 'team', 'teams', 'stable' ), true ) ) {
                            if ( function_exists( 'jjc_mh_expand_team_to_members' ) ) {
                                ...
                                $team_members_meta = get_post_meta( $w, 'team_members', true );
                                if ( is_array( $team_members_meta ) ) foreach ( $team_members_meta as $m ) $check_winners[] = intval( $m );
                            }
                        } else {
                            $check_winners[] = $w;
                        }
                    }
                    if ( in_array( $superstar_id, $check_winners, true ) ) $contains_superstar = true;
                }
            }
        }
        ...
        if ( function_exists( 'wf_is_match_tag' ) ) {
            ...
            $is_tag = false;
            if ( count( $participant_ids ) >= 4 ) $is_tag = true;
            foreach ( $rows as $r ) {
        ...
        // Determine winners for this match (prefer snapshot/winners_expanded, then ACF rows, then legacy wf_winners)
        $match_winner_ids = array();

        // 1) Try snapshot winners_expanded
        $snap_meta = get_post_meta( $mid, 'wf_match_snapshot', true );
        if ( is_string( $snap_meta ) ) {
            $dec = @json_decode( $snap_meta, true );
            if ( is_array( $dec ) && ! empty( $dec['winners_expanded'] ) && is_array( $dec['winners_expanded'] ) ) {
                $match_winner_ids = array_map( 'intval', $dec['winners_expanded'] );
            }
        }

        // 2) If no snapshot winners_expanded, prefer ACF rows with is_winner
        ...
        // 3) Fallback to legacy wf_winners if still empty
        if ( empty( $match_winner_ids ) ) {
            ...
            foreach ( $match_winner_ids as $wid ) {
                ...
                if ( in_array( $w_ptype, array( 'team', 'teams', 'stable' ), true ) ) {
                    if ( function_exists( 'jjc_mh_expand_team_to_members' ) ) {
                        ...
                        $members = array();
                        $tm = get_post_meta( $wid, 'team_members', true );
                        if ( is_array( $tm ) && ! empty( $tm ) ) $members = $tm;
                    }
                    if ( is_array( $members ) && ! empty( $members ) ) {
...
function wf_update_match_snapshot( $match_id ) {
    ...
    if ( function_exists( 'wf_sync_match_participants_meta' ) ) {
        ...
        update_post_meta( $match_id, 'match_participants_expanded', $participant_ids );
    }
    ...
    if ( isset( $snapshot['winners'] ) && is_array( $snapshot['winners'] ) ) {
        foreach ( $snapshot['winners'] as $w ) {
            ...
            if ( in_array( $w_ptype, array( 'team', 'teams', 'stable' ), true ) ) {
                ...
                // fallback: read postmeta 'team_members'
                $tm = get_post_meta( $w, 'team_members', true );
                if ( is_array( $tm ) && ! empty( $tm ) ) {
                    foreach ( $tm as $m ) if ( is_numeric( $m ) ) $expanded_winners[] = intval( $m );
                    continue;
                }
            }
...
/* -----------------------
   Hooks for trash/delete/untrashed to keep counters accurate
   ----------------------- */
